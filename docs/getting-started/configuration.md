...
| `analyticsEnabled` | `boolean` | `true` on Node | Read telemetry reported to Amplitude (off in browsers) |
...
| `statsApiBaseUrl` | `string` | `https://api.stats.usecelina.xyz` | Override on-chain hash reporting URL (`CELINA_STATS_API_URL`) |
